package com.svalero.happDeporte.Util;

public class Literal {

    public static final String LITERAL_NOT_BLANK= "El campo no puede estar en blanco";
    public static final String LITERAL_NOT_NULL= "El campo es obligatorio";
    public static final String ROL_DEFAULT= "USER";
    public static final String BOOLEAN_DEFAULT= "FALSE";
    public static final String LITERAL_BEGIN_GETUSER = "Begin getUser";
    public static final String LITERAL_END_GETUSER = "End getUser";

}
